...
public class GameLogic {
	...
	public boolean isHandPlayable(int playerIndex) {
		ArrayList<BaseCard> hand = this.getPlayerHand(playerIndex);
		if (!hand.isEmpty()) {
	...
	public ArrayList<BaseCard> draw(int amount) {
		// TODO Implement here
		int loop = Math.min(amount, this.getDeck().size());
		ArrayList<BaseCard> addNew = new ArrayList<BaseCard>();
		for (int i = 0; i < loop; i++) {
			addNew.add(this.getDeck().get(0));
