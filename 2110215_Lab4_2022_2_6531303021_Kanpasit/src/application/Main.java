...
public class Main {
	...
	public static void startGame() {
		...
		while(isGameActive) {
			...
			System.out.println((kitchenList.size()+1)+") Serve the Customer");
			System.out.println("=============================");
			...
			if(choice>0&&choice<kitchenList.size()+1) {
				...
				if(serveCustomer()) {
					...
					System.out.println("You cannot serve the customer with that!");
				}
			}else{
	...
	public static void populateCustomer() {
		...
		for(int i=0;i<LogicUtil.getRandomNumber(3)+1;i++) {
			customerList.add(new Customer(LogicUtil.getRandomIngredientOrder(), LogicUtil.getRandomNumber(3)+1));
		}
	}
	...
	public static boolean serveCustomer() {
		...
		if(player.isHandEmpty() || !(currentHand instanceof Dish)) {return false;}
